...
function PhonePreview({project, locale}) {
  ...
  return <div className="phone-shell"><div className="phone-stage">
    ...
    <div className="preview-cta"><img src={`/api/qr?url=${encodeURIComponent(dest.url)}`}/><div><b>{copy.ctaTitle}</b><span>{dest.handle}</span></div></div>
...
function App() {
  ...
  const tabs = [['data','Dữ liệu'],['script','Kịch bản'],['voice','Giọng đọc'],['retention','Giữ chân'],['cta','CTA / Brand'],['export','Render']];
      ...
      <div className="workspace">
        <section className="editor">
          ...
          <div className="panel">
            ...
            {tab==='cta'&&<><div className="locale-switch"><button className={locale==='vi'?'active':''} onClick={()=>setLocale('vi')}>🇻🇳 Zalo</button><button className={locale==='en'?'active':''} onClick={()=>setLocale('en')}>🌍 Telegram</button></div><h1>Điểm đến {locale==='vi'?'Việt Nam':'Global'}</h1><div className="destination-card"><img src={`/api/qr?url=${encodeURIComponent(dest.url)}`}/><div><Field label="Kênh" value={dest.channel} onChange={(v)=>update(`core.destinations.${locale}.channel`,v)}/><Field label="Handle" value={dest.handle} onChange={(v)=>update(`core.destinations.${locale}.handle`,v)}/><Field label="URL đích" value={dest.url} onChange={(v)=>update(`core.destinations.${locale}.url`,v)}/></div></div><div className="route"><span>VI</span><b>→ Zalo</b><i/><span>GLOBAL</span><b>→ @goldfather_fxhub</b></div></>}
