...
export const ContentChart:React.FC<{frame:number;mode:string}> = ({frame,mode}) => {
...
export const UniversalContentShort:React.FC<UniversalProps> = ({locale}) => {
  ...
  return <AbsoluteFill style={{background:C.bg,color:C.white,fontFamily:TEXT_FONT,overflow:'hidden'}}>
    ...
    {sec<35&&<div style={{position:'absolute',inset:'145px 64px 250px'}}>
      ...
      <div style={{height:mode==='indicator'?650:650,marginTop:25,background:'rgba(7,28,32,.86)',border:`1px solid ${C.line}`,borderRadius:24,padding:14}}><ContentChart frame={frame} mode={mode}/>{mode==='indicator'&&<RSI/>}</div>
